...
export default {
  ...
  refresh: (data?: FireRiskRefreshBody) =>
    apiClient.post<FireRiskRefreshData>(`${serviceFireRiskPath}/refresh`, data ?? {}),

  /** GET /fire-risk/snapshots/:id/districts — per-district raster artifacts. */
  getDistrictExports: (snapshotId: number | string) =>
    apiClient.get<FireRiskDistrictExportsData>(
  ...
  publishSnapshotRaster: (id: number | string, force?: boolean) =>
    apiClient.post<FireRiskPublishRasterData>(
